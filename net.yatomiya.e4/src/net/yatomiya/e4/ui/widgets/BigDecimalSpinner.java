...
public class BigDecimalSpinner extends Composite {
    ...
    private void textEntered() {
        ...
        if (!text.getText().equals(v.toString())) {
            force = true;
        }

        updateSelection(v, force, true);
    }
    ...
    private void up() {
        updateSelection(getSelection().add(increment), false, true);
    }

    private void down() {
        updateSelection(getSelection().subtract(increment), false, true);
    }
    ...
    public void setSelection(BigDecimal v) {
        updateSelection(v, true, false);
    }

    private void updateSelection(BigDecimal newValue, boolean force, boolean notifyEvent) {
        ...
        if (!text.getText().equals(newValue.toString())) {
            String newText;
            if (intMode) {
                newText = String.valueOf(newValue.intValue());
            } else {
                newText = newValue.toPlainString();
            }
            text.setText(newText);
        }

        boolean changed = selection == null || selection.compareTo(newValue) != 0;
        selection = newValue;
    ...
    public void setMaximum(BigDecimal max) {
        this.max = max;
        updateSelection(selection, true, false);
    }

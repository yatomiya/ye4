...
/**
 * GridLayout which does not wrap children, set numColumns automatically to children size if horizontal, 1 if vertical.
 * Comments is copied from GridLayout, not updated.
 */
...
protected boolean flushCache (Control control) {
	...
	if (data != null) ((LinearData) data).flushCache ();
	return true;
}

LinearData getData (Control [][] grid, int row, int column, int rowCount, int columnCount, boolean first) {
	Control control = grid [row] [column];
	if (control != null) {
		LinearData data = (LinearData) control.getLayoutData ();
...
Point layout (Composite composite, boolean move, int x, int y, int width, int height, boolean flushCache) {
	...
	for (int i=0; i<count; i++) {
		...
		LinearData data = (LinearData) child.getLayoutData ();
		if (data == null) child.setLayoutData (data = new LinearData ());
	...
	for (int j=0; j<columnCount; j++) {
		for (int i=0; i<rowCount; i++) {
			LinearData data = getData (grid, i, j, rowCount, columnCount, true);
			if (data != null) {
				int hSpan = Math.max (1, Math.min (data.horizontalSpan, columnCount));
				if (hSpan == 1) {
		...
		for (int i=0; i<rowCount; i++) {
			LinearData data = getData (grid, i, j, rowCount, columnCount, false);
			if (data != null) {
				int hSpan = Math.max (1, Math.min (data.horizontalSpan, columnCount));
				if (hSpan > 1) {
				...
				if (data != null) {
					if (data.heightHint == SWT.DEFAULT) {
						...
						if ((currentWidth != data.cacheWidth && data.horizontalAlignment == SWT.FILL) || (data.cacheWidth > currentWidth)) {
							...
							if (flush == null) flush = new LinearData [count];

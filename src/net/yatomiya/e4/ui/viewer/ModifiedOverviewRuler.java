 ...
 ******************************************************************************/
// see comment with "net.yatomiya.e4"
 ...
 * @since 2.1
 */
public class ModifiedOverviewRuler implements IOverviewRulerExtension, IOverviewRuler {
...
//			drawBevelRect(e.gc, r.x, r.y, r.width -1, r.height -1, d.getSystemColor(SWT.COLOR_WIDGET_NORMAL_SHADOW), d.getSystemColor(SWT.COLOR_WIDGET_HIGHLIGHT_SHADOW));
	...
	static class WidgetInfos {
		...
		public WidgetInfos(StyledText textWidget, Canvas canvas) {
            ...
            int partialTopIndex= getPartialTopIndex(textWidget);
        ...
        // net.yatomiya.e4
        // ModifiedOverviewRuler
        // 下記の問題に対処するため、 OverviewRuler を抜き出してパッチを当てた。
        static int getPartialTopIndex(StyledText st) {
            int top= st.getTopIndex();
            int pixels= st.getLinePixel(top);

            // FIXME remove when https://bugs.eclipse.org/bugs/show_bug.cgi?id=123770 is fixed
            if (pixels == -st.getLineHeight(st.getOffsetAtLine(top))) {
                top++;
                pixels= 0;
            }
            ...
            if (pixels > 0)
                top--;
            // ModifiedOverviewRuler
            // 上の処理で top が0以下になってしまうことがある。マイナスだと呼び出し側でエラーになる。
            if (top < 0)
                top = 0;
            // Nicherry
	 ...
	 */
	public ModifiedOverviewRuler(IAnnotationAccess annotationAccess, int width, ISharedTextColors sharedColors) {
	 ...
	 * @since 3.4
	 */
	public ModifiedOverviewRuler(IAnnotationAccess annotationAccess, int width, ISharedTextColors sharedColors, boolean discolorTemporaryAnnotation) {

...
public class StyleNode implements TreeNode<StyleNode>, IRegion {
    ...
    private Map<StyleAttribute, Object> cascadedAttrMap;
    ...
    public StyleNode(StyleTag tag) {
        ...
        cascadedAttrMap = Collections.EMPTY_MAP;
    ...
    private void doReplaceChild(int index, int removeLength, List<StyleNode> replaceNodeList) {
        ...
        if (removeLength > 0) {
            ...
            replaceRegion = new MutableRegion(first.getOffset(), last.getOffset() + last.getLength() - first.getOffset());
        } else {
            int offset;
            if (children.size() == 0) {
                offset = getOffset();
            ...
            replaceRegion = new MutableRegion(offset, 0);
        }
    ...
    protected void cascadeAttributeAll() {
        visitNodeTree(node -> {
                if (node.cascadedAttrMap.size() > 0)
                    node.cascadedAttrMap.clear();

                Set<StyleAttribute> set = new HashSet();
                if (node.getParent() != null && node.getParent().cascadedAttrMap.size() > 0)
                    set.addAll(node.getParent().cascadedAttrMap.keySet());
    ...
    protected void evalCascadedAttribute(StyleAttribute attr) {
        if (!attr.isCascadable()) {
            cascadedAttrMap.remove(attr);

            if (cascadedAttrMap.size() == 0)
                cascadedAttrMap = Collections.EMPTY_MAP;
            return;
        }
        ...
        if (getParent() != null)
            cascadedValue = getParent().cascadedAttrMap.get(attr);
        ...
        if (value == null) {
            ...
            if (cascadedAttrMap == Collections.EMPTY_MAP)
                cascadedAttrMap = new SmallMap<>();

            cascadedAttrMap.put(attr, value);
        }
    }
    ...
    public Object getCascadedAttribute(StyleAttribute attr) {
        return cascadedAttrMap.get(attr);
    }

    public Map<StyleAttribute, Object> getCascadedAttributeMap() {
        return Collections.unmodifiableMap(cascadedAttrMap);
    }
    ...
    public void setDataAll(Map<Object, Object> map) {
                ...
                if (isEnable) {
                    if (isActive()) {
                        ...
                        if (aboveNode != null) {
                            ...
                            doDocumentReplace(new MutableRegion(offset, 0), nodeText);
                        }
                    }
                } else {
                    if (oldActive) {
                        doDocumentReplace(new MutableRegion(getOffset(), getLength()), "");
                    }
                }
            }
        }
        ...
        visitNodeTree(new StyleNodeVisitor() {
                ...
                public boolean visit(StyleNode node) {
                    ...
                    if (showCascadedAttribute) {
                        for (Map.Entry<StyleAttribute, Object> entry : node.cascadedAttrMap.entrySet()) {
                            sb.append(String.format("%s=\'%s\' ", entry.getKey().getName(), entry.getValue().toString(), 10));
                        }
                    }

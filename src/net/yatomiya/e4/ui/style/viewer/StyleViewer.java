...
public class StyleViewer extends ProjectionViewer {
    ...
    private StyleMouseManager nodeMouseManager;
    ...
    public StyleViewer(Composite parent, int style, IVerticalRuler vruler, IOverviewRuler oruler) {
        ...
        nodeMouseManager = new StyleMouseManager(this);

        nodeMouseManager.addListener(new CursorChanger());

        popupManager = new PopupWindowManager(this);

        funcMap = new HashMap<>();
        nodeMouseManager.addListener(new FunctionExecutor());

        addEventFunction("open_link", (viewer, node, event) -> openLink(node));
    ...
    public IRegion getViewportRegion() {
        ...
        return new MutableRegion(start, end - start + 1);
    }
    ...
    protected void syncAnnotationToRegionNode() {
        ProjectionAnnotationModel am = getProjectionAnnotationModel();
        if (am != null) {
            am.removeAllAnnotations();
//            ProjectionAnnotation a = new ProjectionAnnotation();
//            am.addAnnotation(a, new Position(30, 200));
        }
    }
    ...
    public void runWithNoRedraw(Runnable runner) {
        ...
        // workaround: 選択範囲が 0-0 のときのみ、強制的に選択範囲を元に戻す
        // 選択範囲変更のイベントが発生してしまうが大丈夫か。
        ...
        if (oldSelectedRange.x == 0 && oldSelectedRange.y == 0) {
    ...
    protected void openLink(StyleNode node) {
        String urlStr = null;
...
*/
    ...
    class FunctionExecutor extends StyleMouseAdapter {
        @Override
        public void onEnter(StyleViewer viewer, StyleNode node, MouseEvent event) {
            executeFunc(StyleAttribute.ATTRIBUTE_ONENTER, viewer, node, event);
        }

        @Override
        public void onExit(StyleViewer viewer, StyleNode node) {
            executeFunc(StyleAttribute.ATTRIBUTE_ONEXIT, viewer, node, null);
        }

        @Override
        public void onMove(StyleViewer viewer, StyleNode node, MouseEvent event) {
            executeFunc(StyleAttribute.ATTRIBUTE_ONMOVE, viewer, node, event);
        }

        @Override
        public void onClick(StyleViewer viewer, StyleNode node, MouseEvent event) {
            executeFunc(StyleAttribute.ATTRIBUTE_ONCLICK, viewer, node, event);
        }

        @Override
        public void onHover(StyleViewer viewer, StyleNode node, MouseEvent event) {
            executeFunc(StyleAttribute.ATTRIBUTE_ONHOVER, viewer, node, event);
        }

        private void executeFunc(StyleAttribute attr, StyleViewer viewer, StyleNode node, MouseEvent event) {
            String name = (String)node.getCascadedAttribute(attr);
            if (JUtils.isNotEmpty(name)) {
                ...
                if (func != null) {
                    func.execute(viewer, node, event);
                }
            }
        }
    }

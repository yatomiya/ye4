...
public class IOUtils {
    ...
    public static boolean checkAndCreateDirectory(File dir) {
        if (dir.exists()) {
            if (dir.isDirectory())
                return true;
            else
    ...
    public static Object readObject(InputStream ins, boolean gzip) throws IOException {
        if (gzip)
            ins = ungzip(ins);
        return deserializeObject(readString(ins));
    }
    ...
    private static void initializeXStream() {
        ...
        ClassLoader cl = AppUtils.get(Application.class).getClass().getClassLoader();
